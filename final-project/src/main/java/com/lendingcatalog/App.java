...
public class App {
    ...
    private void initialize() {
        try {
            // Read members data from members.dat
            List<Member> members = readMembersFromFile(FILE_BASE_PATH + "members.dat");

            // Process each member
            for (Member member : members) {
                String itemsFilename = FILE_BASE_PATH + member.getItemsFilename();

                // Read items data from the member's items file
                List<String> itemLines = FileStorageService.readContentsOfFile(itemsFilename);

                // Create a list to store the catalog items for the member
                List<CatalogItem> memberItems = new ArrayList<>();

                // Process each item line
                for (String itemLine : itemLines) {
                    String[] fields = itemLine.split(FIELD_DELIMITER);

                    // Check if the line has the correct number of fields
                    if (fields.length == 4) {
                        String itemType = fields[0];

                        // Instantiate the appropriate catalog item based on the item type
                        CatalogItem catalogItem;
                        if (itemType.equals("book")) {
                            catalogItem = new Book(UUID.randomUUID().toString(), fields[1], fields[2], LocalDate.parse(fields[3]));
                        } else if (itemType.equals("movie")) {
                            catalogItem = new Movie(UUID.randomUUID().toString(), fields[1], fields[2], LocalDate.parse(fields[3]));
                        } else if (itemType.equals("tool")) {
                            catalogItem = new Tool(UUID.randomUUID().toString(), fields[1], fields[2], Integer.parseInt(fields[3]));
                        } else {
                            // Invalid item type, skip this line
                            continue;
                        }

                        // Register the item and add it to the member's items list
                        catalogItem.registerItem();
                        memberItems.add(catalogItem);
                    }
                }

                // Add the member's items list to the catalog map
    ...
    private List<Member> readMembersFromFile(String filename) throws FileStorageException {
        ...
        for (String line : lines) {
            ...
            if (fields.length == 3) {
                ...
                String itemsFilename = fields[2];

                members.add(new Member(firstName, lastName, itemsFilename));
            }
        }
        return members;
    }
